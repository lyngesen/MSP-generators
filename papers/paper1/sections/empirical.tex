...
\subsection{Empirical study of information-sharing between subproblems}
...
\end{align}
The constraints \Cref{constr:MIP-subset-chosen} together with \Cref{constr:MIP-combination-single} also ensures that at least one solution is chosen to each subproblem $\left( \sum_{i \in I^s} x^s_i,  \forall s \in \S \right)$.
...
A pitfall of MGS1 and MGS2 is that the metrics they produce are not unique.
