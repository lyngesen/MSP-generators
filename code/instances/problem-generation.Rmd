---
title: "Generation of instance files for subproblems and combining them into problems"
...
deleteFiles <- function(delete = TRUE) {
  paths <- fs::dir_ls("subproblems/")
  paths <- str_subset(paths, "sp-")  # focus on sp files
  # if (delete == "m") {
...
The problems generated consists of $2 \leq m\leq 5$ subproblems and is generated so provide a good test bed for a research question. The naming convention is `prob-<p>-<cards>-<gen-methods>_<m>_<id>.json`
...
pS <- 2:5
mS <- 2:5
samplesS <- c(50, 100, 200, 300, 600) # nd points set size
methodS <- c("u", "l", "m", "ul")
...
dat1 <- dat |> 
  filter(card != 10, card != 150, card != 600) 
...
tikz("~/Library/CloudStorage/Dropbox/Apps/Overleaf/paper1/figures/se-card.tex", width = 8, height = 3)
pt1
dev.off()
tikz("~/Library/CloudStorage/Dropbox/Apps/Overleaf/paper1/figures/se-p.tex", width = 8, height = 3)
pt2
dev.off()
```
...
#' Custom theme 
#'
#' @return The theme object for ggplot
...
path <- "subproblems/sp-2-50-"
lst <- jsonlite::read_json(str_c(path, "l_1.json"), simplifyVector = T)
dat <- lst$points |> mutate(method = "\\mL")
lst <- jsonlite::read_json(str_c(path, "m_1.json"), simplifyVector = T)
dat <- bind_rows(dat, lst$points |> mutate(method = "\\mM"))
lst <- jsonlite::read_json(str_c(path, "u_1.json"), simplifyVector = T)
dat <- bind_rows(dat, lst$points |> mutate(method = "\\mU"))
...
pt <- dat |> 
  ...
  coord_fixed() + 
  labs(x = "$y_1$", y = "$y_2$", shape = NULL, color = NULL, label = NULL) +
  theme_academic()
pt

tikz("sp-50-lmu.tex", width = 8, height = 3)
pt
dev.off()
```
