...
saveSubproblems <- function(pS, samplesS, dub, overwrite = TRUE) {
  ...
  for (p in pS) {
    for (s in samplesS) {
      for (d in 1:dub) {
        pathU <- str_c("subproblems/sup-", p, "-", s, "-u", "_", d, ".json")
        pathM <- str_c("subproblems/sup-", p, "-", s, "-m", "_", d, ".json")
        pathL <- str_c("subproblems/sup-", p, "-", s, "-l", "_", d, ".json")
...
deleteFiles <- function(delete = TRUE) {
  paths <- fs::dir_ls("subproblems/")
  paths <- paths[-1]  # remove statistics file
  if (delete == "m") {
...
That is a subproblem can be identified using filename `sub-<p>-<card>-<gen-method>_<id>.json` where `id` denote the instance id for the same configuration. 
...
The problems generated consists of $m\leq 5$ subproblems and is generated so provide a good test bed for a research question.
...
saveProblems <- function(pS, mS, samplesS, methodS, dub, overwrite = TRUE) {
  paths <- fs::dir_ls("subproblems/")
  paths <- paths[-1]  # remove statistics file
  # prob-<p>-<samples>-<m>-<sphere-method>-<sub-ids>.json|.
  for (p in pS) {
    for (s in samplesS) {
      for (method in methodS) {
        if (method != "ul") {
          subS <- str_c("subproblems/sup-", p, "-", s, "-", method)
          curPaths <- str_subset(paths, subS)
        } 
        if (method == "ul") {
          subS <- str_c("subproblems/sup-", p, "-", s, "-u")
          curPathsU <- str_subset(paths, subS)
          subS <- str_c("subproblems/sup-", p, "-", s, "-l")
...
deleteFilesProb()
saveProblems(pS, mS, samplesS, methodS, dub, overwrite = T)
deleteFilesProb()
```
