...
```
  ...
  * `r datFull |> filter(is.na(unsupported) & !is.na(extreme)) |> nrow()`/`r datFull |> filter(!is.na(card)) |> nrow() - datFull |> filter(is.na(extreme) & !is.na(card)) |> nrow()` have not been fully classified (only classified extreme).
...
tikz("yn-spcard.tex", width = 10, height = 10)
pt1
dev.off()
