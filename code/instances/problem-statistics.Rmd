...
```
  ...
  * `r datFull |> filter(!is.na(card)) |> nrow()`/`r nrow(datFull)` problems have been solved.
  * `r nrow(dat)`/`r length(files)` problems have 5 instances solved for each configuration.
  * `r datFull |> filter(is.na(extreme) & !is.na(card)) |> nrow()`/`r nrow(datFull)` have not been classified.
  * `r datFull |> filter(is.na(unsupported) & !is.na(extreme)) |> nrow()`/`r nrow(datFull)` have not been fully classified (only classified extreme).
